...
export default class SKPacket{
    ...
    encode(properties:any):any{
        ...
        //客户端加密
        let encodebuf = new ByteBuffer();
        let max=buffer.limit;
        for(let i=0;i<max;i++)
        {
            let index = buffer.readUint8();
            encodebuf.writeByte(SKPacket.g_send_map[index]);
        }
        encodebuf.flip();
        //-----------
        let result=encodebuf.toBuffer();;
        return result;
    }
